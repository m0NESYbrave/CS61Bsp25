...
public class PalindromeStage implements AdventureStage {
    ...
    private static IntList reverseList(IntList l) {
        ...
        while (l != null) { // if l.rest != null, always lose a item, so always not a palindrome
    ...
    private static IntList digitsToIntList(String s) {
        ...
        for (int i = s.length(); i > 0; i--) { // not i++
            a[s.length() - i] = Character.getNumericValue(s.charAt(s.length() - i)); //i begins with length() so charAt(i) will fail
        }
        return IntList.of(a);
    }
